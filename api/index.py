...
def get_name_region_from_reward(access_token):
    """Get user info from reward API with multiple fallback methods"""
    
    # Method 1: Original reward API
    try:
        ...
        logger.error(f"Reward API method 1 failed: {e}")
    
    # Method 2: Alternative reward API endpoint
    try:
        url = "https://reward.ff.garena.com/api/auth/inspect_token/"
        headers = {
        ...
        resp = requests.get(url, headers=headers, verify=False, timeout=15)
        logger.debug(f"Alternative reward API response: {resp.status_code}")
        
        if resp.status_code == 200:
            data = resp.json()
            uid = data.get("uid") or data.get("user_id") or data.get("id")
            if uid:
                return uid, data.get("name"), data.get("region")
    except Exception as e:
        logger.error(f"Reward API method 2 failed: {e}")
    
    # Method 3: Get user info from Garena API
    try:
        url = "https://api.garena.com/user/info"
        headers = {
            "Authorization": f"Bearer {access_token}",
            "user-agent": "Mozilla/5.0 (Linux; Android 13) AppleWebKit/537.36"
        }
        resp = requests.get(url, headers=headers, verify=False, timeout=15)
        logger.debug(f"Garena API response: {resp.status_code}")
        ...
        logger.error(f"Garena API failed: {e}")
    
    # Method 4: Extract from access token (if JWT)
    try:
        import base64
    ...
    # If all methods fail, try to get uid from guest login response
    logger.error("All reward API methods failed")
...
def get_openid_from_shop2game(uid):
    ...
    try:
        ...
        url = "https://api.shop2game.com/v1/user/openid"
        payload = {"uid": str(uid), "app_id": 100067}
        resp = requests.post(url, headers=headers, json=payload, verify=False, timeout=15)
        if resp.status_code == 200:
            data = resp.json()
            open_id = data.get("open_id") or data.get("openid")
...
def bmw_endpoint():
    ...
    try:
        ...
        # Step 2: Get user info (with multiple methods)
        logger.info("Fetching user info from reward API...")
        uid_found, name, region = get_name_region_from_reward(acc_token)
        
        # If reward API fails, try to get uid from the access token or use provided uid
        if not uid_found:
            logger.warning("Reward API failed, trying to use provided uid...")
            uid_found = uid
            
            # Try to get name from a different source
            try:
                # Try to decode JWT token for name
                import base64
                token_parts = acc_token.split('.')
                if len(token_parts) >= 2:
                    payload = base64.b64decode(token_parts[1] + '==').decode('utf-8')
                    data = json.loads(payload)
        ...
        # Step 3: Get open_id (try multiple methods)
        logger.info("Fetching open_id...")
        open_id_from_shop = get_openid_from_shop2game(uid_found)
        
        # If shop2game fails, use the open_id from guest login
        ...
        if not open_id_from_shop:
            return jsonify({
                "status": "error", 
                "message": "Could not fetch open_id from any source",
                "step": "open_id_failed"
            }), 400
